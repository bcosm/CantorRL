...
CHECKPOINT_FILE_TEMP_STEM = './rbergomi_generator_checkpoint_temp' # Stem for temp file
...
def generate_paths_and_options(historical_prices_np_main, num_paths_main, r_main, dt_main, seed_main_val=None):
    ...
    actual_checkpoint_temp_file = CHECKPOINT_FILE_TEMP_STEM + ".npz" # NumPy will add .npz
    ...
    with tqdm(range(start_step, N_STEPS + 1), initial=start_step-1, total=N_STEPS, desc="Main Simulation Progress", unit="day") as pbar_days:
        for j_main_val in pbar_days:
            ...
            # Corrected temporary file name handling for saving
            np.savez_compressed(CHECKPOINT_FILE_TEMP_STEM, # Save to stem, NumPy adds .npz
            ...
            os.replace(CHECKPOINT_FILE_TEMP_STEM + ".npz", CHECKPOINT_FILE) # Replace with the actual .npz file

    print(f"Total time spent in batched option pricing calls: {total_option_pricing_time_tracker:.2f} seconds.")
    
    if start_step > N_STEPS : # Check if loop finished all steps
        if os.path.exists(CHECKPOINT_FILE):
            try:
