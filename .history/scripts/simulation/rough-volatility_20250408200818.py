...
class RoughVolatility:
    ...
    """
    Estimate the vol-of-vol parameter (eta) in a more sophisticated way by:
    
    1. Computing a rolling realized variance over a specified window (default 20 days).
    2. Taking the logarithm of these realized variances.
    3. Calculating the daily changes (differences) in the log-realized variance.
    4. Estimating eta as the standard deviation of these log differences, and then annualizing it.
    
    Parameters:
        logrets (array-like): The vector of log returns.
        H (float): The Hurst exponent (not used directly here but kept for interface consistency).
        window (int): The rolling window size in days to compute the realized variance.
        
    Returns:
        float: The annualized estimate of the vol-of-vol parameter eta.
    """
    import numpy as np
    import math

    # Fallback: if not enough data for rolling estimation, use the basic approach.
    if len(logrets) < window:
        stdev = math.sqrt(np.var(logrets, ddof=1))
        return stdev * 2.0

    # Compute rolling realized variance as the average of squared log returns in the window.
    realized_var = []
    ...
    # Compute the log differences of the realized variance estimates.
    log_rv = np.log(realized_var)
    log_diff = np.diff(log_rv)
    
    # Estimate the daily volatility-of-volatility as the std. deviation of log differences.
    daily_eta = np.std(log_diff, ddof=1)
    
    # Annualize the estimate (assuming 252 trading days per year).
