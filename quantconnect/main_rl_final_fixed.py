...
class RLHedgingFinalAlgorithm(QCAlgorithm):
    ...
    def Rebalance(self, data):
        ...
        # Get RL model prediction (now returns continuous actions [calls, puts])
        actions = self.GetModelPrediction()
        
        # Execute trades based on continuous actions
        self.ExecuteAction(actions, calls, puts)
    ...
    def GetModelPrediction(self):
        """Get prediction from RecurrentPPO model or use fallback"""
        if self.model is None or len(self.price_history) < 10:
            # Fallback: simple momentum-based actions
            self.fallback_predictions += 1
            if len(self.price_history) >= 2:
                price_change = self.price_history[-1] - self.price_history[-2]
                if price_change > 0:
                    return [0.3, 0.0]  # Buy some calls
                else:
                    return [0.0, 0.3]  # Buy some puts
        ...
        try:
            ...
            # Get model prediction with LSTM state
            with torch.no_grad():
                obs_tensor = torch.FloatTensor(obs).unsqueeze(0).unsqueeze(0)  # (batch, seq, features)
                actions, self.hidden_state = self.model(obs_tensor, self.hidden_state)
                actions_np = actions.squeeze().numpy()
            
            self.model_predictions += 1
            return actions_np  # Returns [call_action, put_action] in range [-1, 1]
            
        except Exception as e:
            self.fallback_predictions += 1
            return [0.0, 0.0]  # Default no action
    ...
    def BuildObservation(self):
        ...
        spy_position = self.Portfolio[self.spy_symbol].Quantity / 1000.0
    ...
    def ExecuteAction(self, actions, calls, puts):
        """Execute trading action based on continuous RL model output"""
        call_action, put_action = actions[0], actions[1]
        
        # Scale actions to contract quantities (actions are in [-1, 1])
        call_contracts = int(abs(call_action) * 3)  # Up to 3 contracts
        put_contracts = int(abs(put_action) * 3)    # Up to 3 contracts
        
        # Buy calls if call_action > 0.2 threshold
        if call_action > 0.2 and calls and call_contracts > 0:
            order_id = self.MarketOrder(calls[0].Symbol, call_contracts)
            if order_id:
                self.successful_trades += 1
                self.Debug(f"MODEL CALL BUY: {calls[0].Strike} strike x{call_contracts}")
                
        # Buy puts if put_action > 0.2 threshold
        if put_action > 0.2 and puts and put_contracts > 0:
            order_id = self.MarketOrder(puts[0].Symbol, put_contracts)
            if order_id:
                self.successful_trades += 1
                self.Debug(f"MODEL PUT BUY: {puts[0].Strike} strike x{put_contracts}")
